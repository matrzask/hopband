...
void example_lvgl_port_update_callback(lv_disp_drv_t *drv)
{
    ...
    case LV_DISP_ROT_90:
        // Rotate LCD display
        ESP_LOGI(TAG_LVGL, "Rotate LCD display 90 degrees");
...
void LVGL_Init(void)
{
    ...
    buf1 = (lv_color_t *)heap_caps_malloc(LVGL_BUF_LEN * sizeof(lv_color_t), MALLOC_CAP_DMA);
    buf2 = (lv_color_t *)heap_caps_malloc(LVGL_BUF_LEN * sizeof(lv_color_t), MALLOC_CAP_DMA);
    assert(buf1 && buf2);

    lv_disp_draw_buf_init(&disp_buf, buf1, buf2, EXAMPLE_LCD_H_RES * 5); // initialize LVGL draw buffers
    ...
    disp_drv.rotated = LV_DISP_ROT_270;                         // 图像旋转                                                            // Vertical axis pixel count
    ...
    disp_drv.sw_rotate = 1;                               // Use software rotation (slower) if the display is rotated
